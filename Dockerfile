...
RUN mkdir -p "$SINUS_DIR" "$TS3_DIR" && \
    wget -qO- http://frie.se/ts3bot/sinusbot-$SINUS_VERSION.tar.bz2 | \
    tar -xjf- -C "$SINUS_DIR" && \
    wget -qO- https://www.sinusbot.com/pre/sinusbot-0.9.10-e1fdee3.tar.bz2 | \
    tar -xjf- -C "$SINUS_DIR"

RUN wget "http://cloud.ricosw.de/tellur/files/teamspeak.tar" && \
...
VOLUME ["$SINUS_DIR/scripts", "/var/log/sinusbot"]
EXPOSE 8087
